...
func TestClient_AcknowledgeNotification(t *testing.T) {
	...
	t.Run("acknowledge notification", func(t *testing.T) {
		...
		err = client.AcknowledgeNotification(testPoolName, n.Event.ID, nil)
		if err != nil {
			t.Fatalf("Failed to acknowledge notification: %v", err)
		}
	})

	t.Run("acknowledge notification with subsequent events", func(t *testing.T) {
		err := client.NotifyEvent("test-source", "test-subject", "test-type", map[string]string{"key": "value"}, nil)
		if err != nil {
			t.Fatalf("Failed to notify event: %v", err)
		}
		...
		err = client.AcknowledgeNotification(testPoolName, n.Event.ID, []orbit.Event{{
			Source:  "test-source",
		...
		if err != nil {
			t.Fatalf("Failed to acknowledge notification with subsequent events: %v", err)
		}
